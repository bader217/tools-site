...
function getMessage(type) {
    ...
    if (type === "success" && params.get("success") === "1")
        return "Tool added successfully.";

    // ERRORS
    if (type === "error" && params.get("error") === "empty")
        return "Please fill in all fields.";

    if (type === "error" && params.get("error") === "invalid")
        return "Invalid category or type.";

    if (type === "error" && params.get("error") === "db")
...
function renderTable() {
    ...
    if (!tools.length) {
        tbody.innerHTML =
            '<tr><td colspan="6" class="admin-tbl-loading">No tools found.</td></tr>';
        return;
    }
    ...
    tbody.innerHTML = tools
        .map((tool, index) => {
            return (
                ...
                "</a></td>" +
                "<td>" +
                "<form method='POST' action='/admin/tools/delete/" +
                tool.id +
...
async function loadTools() {
    ...
    if (tbody) {
        tbody.innerHTML =
            '<tr><td colspan="6" class="admin-tbl-loading">LOADING...</td></tr>';
    }
    ...
    try {
        ...
        if (tbody) {
            tbody.innerHTML =
                '<tr><td colspan="6" class="admin-tbl-loading">Failed to load tools.</td></tr>';
        }
    }
}
